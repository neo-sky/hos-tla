...
use crate::events::{self, TlaActivatedEvent};
use crate::fees;
use crate::types::*;
use crate::{TlaRegistry, TlaRegistryExt};
use near_sdk::json_types::U128;
use near_sdk::{env, ext_contract, near, AccountId, Gas, NearToken, Promise, PublicKey};
...
trait TlaManagerExt {
    fn create_sub_account(&mut self, name: String, owner_key: PublicKey);
}
...
impl TlaRegistry {
    ...
    pub fn activate_tla(&mut self, tla_id: AccountId) -> Result<(), ContractError> {
        ...
        let required = self.fee_config.tla_allocation_fee.saturating_add(rent);
        ...
        let paid_str = required.to_string();
        events::emit(
            "tla_activated",
            &TlaActivatedEvent {
    ...
    pub fn rent_sub_account(
        ...
        let total = rent.saturating_add(self.fee_config.account_creation_deposit);
        ...
        let creation_deposit = NearToken::from_yoctonear(self.fee_config.account_creation_deposit);
    ...
    pub fn renew_tla(&mut self, tla_id: AccountId) -> Result<(), ContractError> {
        ...
        let is_business;
        {
            ...
            if now >= entry.expires_at.saturating_add(GRACE_PERIOD_NS) {
        ...
        let paid_str = rent.to_string();
        events::emit(
            "tla_renewed",
            &events::RenewalEvent {
                entity: tla_id.as_str(),
                new_expires_at,
                paid_yocto: &paid_str,
            },
        );
    ...
    pub fn set_main_wallet(
        ...
        events::emit(
            "main_wallet_updated",
            &events::MainWalletEvent {
                full_name: &key,
                new_wallet: new_wallet.as_str(),
            },
        );
        Ok(())
    }
    ...
    pub fn renew_sub_account(
        ...
        let rent;
        {
            let sub = self
                ...
                .get(&key)
                .ok_or(ContractError::SubAccountNotFound)?;
            if now >= sub.expires_at.saturating_add(GRACE_PERIOD_NS) {
            ...
            if matches!(tla.lifecycle(), LifecycleStatus::Reclaimable) {
                return Err(ContractError::TlaPastGracePeriod);
            }
