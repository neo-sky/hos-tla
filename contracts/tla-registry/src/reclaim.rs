...
use near_sdk::{
    env, ext_contract, is_promise_success, near, AccountId, FunctionError, Gas, Promise,
    PromiseError, PromiseOrValue,
};

const GAS_FOR_LOCKER_SWEEP: Gas = Gas::from_tgas(60);
const GAS_FOR_LOCKER_DELETE: Gas = Gas::from_tgas(30);
...
impl TlaRegistry {
    ...
    pub fn reclaim_sweep_ft(
        ...
        Ok(ext_locker::ext(sub_account)
            .with_static_gas(GAS_FOR_LOCKER_SWEEP)
            .sweep_ft(ft, destination))
    }
    ...
    pub fn reclaim_finalize(
        ...
        if allowlist.is_empty() {
            return Ok(ext_locker::ext(sub_account)
                .with_static_gas(GAS_FOR_LOCKER_DELETE)
                .finalize_delete(destination.clone())
                .then(
        ...
        for i in 0..count {
            ...
            match env::promise_result_checked(i, FT_BALANCE_MAX_LEN) {
                Ok(bytes) => {
                    ...
                    if balance.0 > 0 {
                        let reason = balance.0.to_string();
                        events::emit(
                            "reclaim_finalize_blocked",
                            &FinalizeBlockedEvent {
                                full_name: &key,
                                token: token.as_str(),
                                reason: &reason,
                            },
                ...
                Err(_) => {
                    events::emit(
                        ...
                        &FinalizeBlockedEvent {
                            full_name: &key,
                            token: token.as_str(),
                            reason: "balance_query_unverifiable",
                        },
                    );
                    return PromiseOrValue::Value(());
                }
    ...
    pub fn on_reclaim_finalized(
        ...
        events::emit(
            "sub_account_reclaimed",
            &SubAccountReclaimedEvent {
                full_name: &key,
